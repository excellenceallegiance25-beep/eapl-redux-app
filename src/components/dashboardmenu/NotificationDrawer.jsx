...
import { decrementUnreadCount, triggerNoticeRefresh } from "../../redux/slices/notificationSlice";
import {
  getMarkNoticeAsRead,
  getNoticesList,
  getNotificationList,
...
const NotificationDrawer = ({ open, onClose, isMobile }) => {
  ...
  const fetchNotifications = async (loggedInId) => {
    try {
      ...
      //     message:
      //       "The quarterly meeting schedule has been updated. Please check the new timings.",
      ...
      //     message:
      //       "Your leave request for January 15-17, 2024 has been approved.",
      ...
      //     message:
      //       "Annual team building event scheduled for February 20, 2024.",
      ...
      if (result.type === "EMP_NOTIFICATION_LIST") {
        const notificationList = result.payload.dataList.map(
          (notification) => ({
            ...notification,
          }),
        );
  ...
  const handleNotificationClickFFF = async (notification) => {
  ...
  const markAsReadFFF = async (notificationId, noticeId) => {
    try {
    ...
    try {
        ...
        if (noticeId) {
            ...
            if (result.payload?.success) {
                ...
                // ⭐ TRIGGER NOTICE REFRESH - Add this line
                dispatch(triggerNoticeRefresh());
          ...
          <MenuItem
            onClick={() => {
              if (selectedNotification && !selectedNotification.read) {
                ...
                const noticeId =
                  parsedData.noticeId || parsedData.refId || parsedData.id;
                // console.log("Found noticeId:", noticeId);
