    ...
    try {
      ...
      if (result?.type === "EMP_APPLY_LEAVE_SUCCESS") {
        ...
        return {
          success: true,
          message:
            result.payload?.message || "Leave request submitted successfully",
        }; // Indicate success
      } else {
        hideLoader();
        ...
        return {
          success: false,
          message: result.payload?.message || "Operation failed",
        };
      }
    } catch (error) {
      hideLoader();
  ...
  return (
    <Container maxWidth="xl" disableGutters>
      ...
      >
        ...
        >
          <Button
            variant="contained"
            startIcon={
              <Add sx={{ display: { xs: "none", sm: "inline-flex" } }} />
            }
            onClick={() => {
              setCurrentLeave(null);
              setDialogOpen(true);
      ...
      <Box display="flex" gap={2} mb={2} flexWrap="wrap">
        <Paper
          sx={{ p: 1, flex: 1, minWidth: 150, textAlign: "center" }}
            ...
            <TableBody>
              {paginatedLeaves.length > 0 ? (
                paginatedLeaves.map((leave) => {
                  ...
                  // Get colors based on status (same logic as getStatusChip)
                  ...
                  const rowColors = getRowColors(leave.status);
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{
                        cursor: "pointer",
                        backgroundColor: rowColors.bgcolor,
                        borderLeft: `3px solid ${rowColors.borderColor}40`,
                        transition: "all 0.2s ease",
                        "&:hover": {
                          backgroundColor: `${rowColors.bgcolor}DD`, // Darken on hover
