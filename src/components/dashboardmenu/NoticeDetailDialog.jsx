...
const NoticeDetailDialog = ({
  ...
  const hasRead =
    notice.readBy?.some((reader) => reader.employeeID === userID) || false;
  const priorityColor =
  ...
  const readPercentage =
    notice.allEmployees?.length > 0
      ? Math.round(
          ((notice.readBy?.length || 0) / notice.allEmployees.length) * 100,
        )
      : 0;
      ...
      <DialogContent sx={{ pt: 3 }}>
        ...
        >
          <Chip
            ...
            variant="outlined"
          />
          <Chip label={notice.priority} size="small" color={priorityColor} />
          {!notice.isActive && (
            <Chip
              label="Archived"
              size="small"
        ...
        {isAdmin && notice.allEmployees?.length > 0 && (
          <Box sx={{ mt: 3, pt: 3, borderTop: 1, borderColor: "divider" }}>
            <Typography variant="subtitle2" gutterBottom>
              Read Status: {notice.readBy?.length || 0} out of{" "}
              {notice.allEmployees.length} employees ({readPercentage}%)
            </Typography>
            <Box
              sx={{
                width: "100%",
